...
DROP POLICY IF EXISTS "Admin full access on roles" ON roles;
DROP POLICY IF EXISTS "Admin full access on permissions" ON permissions;
DROP POLICY IF EXISTS "Admin full access on role_permissions" ON role_permissions;
DROP POLICY IF EXISTS "Users can view their own profile" ON profiles;
DROP POLICY IF EXISTS "Admin can manage all profiles" ON profiles;

-- Create non-recursive policies for roles table
CREATE POLICY "Admin read access on roles"
ON roles
FOR SELECT
TO authenticated
USING (
  EXISTS (
    ...
    AND profiles.role_id IN (SELECT id FROM roles WHERE name = 'admin')
  )
);

CREATE POLICY "Admin write access on roles"
ON roles
FOR ALL
TO authenticated
USING (
...
CREATE POLICY "Admin access on permissions"
ON permissions
FOR ALL
TO authenticated
USING (
...
CREATE POLICY "Admin access on role_permissions"
ON role_permissions
FOR ALL
TO authenticated
USING (
